...
void set_minlen_nextdenovo(mm_bseq_file_t **fp, const int n_fp, const unsigned minlen){
	int i = 0;
	for (i = 0; i < n_fp; ++i) if (fp[i]->ks) fp[i]->ks->minlen = minlen;
}
