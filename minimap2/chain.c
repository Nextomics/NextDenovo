...
mm128_t *mm_chain_dp_nextdenovo(int max_dist_x, int max_dist_y, int bw, int max_skip, int max_iter, int min_cnt, int min_sc, int is_cdna, int n_segs, int64_t n, mm128_t *a, int *n_u_, uint64_t **_u, void *km)
{ // TODO: make sure this works when n has more than 32 bits
	int32_t k, *f, *p, *t, *v, n_u, n_v;
	int64_t i, j, st = 0;
	uint64_t *u, *u2, sum_qspan = 0;
	...
	//remove minizer with high hits
	if (n > 100000){//TODO TEST 100000
		int32_t px, pm, pi;
		int maxc = 200, maxw = 10;
		float maxf = 0.8;// TODO TEST
		for (i = j = px = k = 0; i < n; ++i){
			pi = (int32_t)a[i].x;
			if (pi != px) {
				if (t[j] > k) k = t[j];
				j ++;
		...
		if (t[j] > k) k = t[j];
		v[j++] = (int32_t)a[i - 1].x + maxw * 2;
		
		// printf("maxk: %d %d ", k,  (int32_t) ((double)k * maxf + .499));
		if (k > maxc){
			k = (int32_t) ((double)k * maxf + .499);
			for (i = j = px = 0, pm = (int32_t)a[0].x; i < n; ++i){
				pi = (int32_t)a[i].x;
				if (pi != px) {
					px = pi;
					j ++;
				}
				// printf("%d %d %d %d %d %d\n",i, a[i].x, pm, v[j], a[i].x + maxw, v[j] < a[i].x + maxw ? 1: 0);
				if (t[j] > k && pi > pm && pi < pm + maxw && v[j] < pi + maxw) a[i].x = -1;
				else pm = pi;//3828291 19757 19756 19780 19767 1 test4.log1.6
				// printf("%d\n", a[i].x);
			}
		}
		memset(t, 0, n * 4);
		// if (1) {
		// 	for (i = 0; i < n; ++i)
		// 		fprintf(stdout, "DD\t%d\t%c\t%d\t%d\t%d\t%d\n", (int32_t)a[i].x, "+-"[a[i].x>>63], 
		// 			(int32_t)a[i].y, (int32_t)(a[i].y>>32&0xff),
		// 				i == 0? 0 : ((int32_t)a[i].y - (int32_t)a[i-1].y) - ((int32_t)a[i].x - (int32_t)a[i-1].x), i);
		// }
		// exit(1);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		if (a[i].x == -1) {
			v[i] = f[i] = p[i] = -1;
			continue;//skip minizer with high hits
		}
		...
		int32_t max_f = q_span, n_skip = 0, min_d;
		int32_t sidi = (a[i].y & MM_SEED_SEG_MASK) >> MM_SEED_SEG_SHIFT;
		while (st < i && (a[st].x == -1 || ri > a[st].x + max_dist_x)) ++st;
		if (i - st > max_iter) st = i - max_iter;
		for (j = i - 1; j >= st; --j) {
			if (a[j].x == -1) continue;
			int64_t dr = ri - a[j].x;
	...
	memset(t, 0, n * 4);
	for (i = 0; i < n; ++i)
		// if (p[i]>=0) printf("score:%d\t%d\t%d\t%d\t%d\t%d\n",i,a[i].x,p[i], a[p[i]].x,f[i],v[i]);
		// else printf("score:%d\t%d\t-1\n",i,a[i].x);	
		if (p[i] >= 0) t[p[i]] = 1;
	for (i = n_u = 0; i < n; ++i)
		if (t[i] == 0 && v[i] >= min_sc)
			++n_u;
	...
	// for (i = 0; i < n_u; i++){
	// 	int m = (int32_t)u[i];
	// 	fprintf(stdout, "RE1\t%d\t%d\t%c\t%d\t%d\t%d\t%d\n",i, (int32_t)a[m].x, "+-"[a[m].x>>63], 
	// 		(int32_t)a[m].y, (int32_t)(a[m].y>>32&0xff),m,(int32_t)(u[i]>>32));
	// 	// printf("max\t%d\t%d\t%d\n",i,(int32_t)u[i],(int32_t)(u[i]>>32));
	// }
	// exit(1);
	...
	*n_u_ = n_u = k, *_u = u; // NB: note that u[] may not be sorted by score here
	...
	for (i = 0, k = 0; i < n_u; ++i) {
		...
		for (j = 0; j < ni; ++j){
			...
			// int m = v[k0 + (ni - j - 1)];
			// fprintf(stdout, "RE2\t%d\t%d\t%c\t%d\t%d\t%d\n",i, (int32_t)a[m].x, "+-"[a[m].x>>63], 
		 // 			(int32_t)a[m].y, (int32_t)(a[m].y>>32&0xff),m);
		}
	}
	kfree(km, v);
