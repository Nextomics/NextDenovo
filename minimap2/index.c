...
mm_idx_t *mm_idx_init_nextdenovo(int w, int k, int is_hpc, int bucket_bits, int n)
{
	mm_idx_t *mi;
	int flag = 0;
	khash_t(str) *h;
	...
	mi->sum_len = n * 50000;
	mi->seq = (mm_idx_seq_t*)kcalloc(mi->km, n, sizeof(mm_idx_seq_t)); // ->seq is allocated from km
	mi->S = (uint32_t*)calloc((mi->sum_len + 7) / 8, 4);
	mi->h = h = kh_init(str);
	return mi;
}

void mm_idx_clean_nextdenovo(mm_idx_t *mi, const int n)
{	
	uint32_t i;
	if (mi->h) kh_clear(str, (khash_t(str)*)mi->h);
	if (mi->B) {
		for (i = 0; i < 1U<<mi->b; ++i) {
			free(mi->B[i].p);
			free(mi->B[i].a.a);
	...
	// if (!mi->km) {
	// 	for (i = 0; i < mi->n_seq; ++i) free(mi->seq[i].name);
	// }
	memset(mi->B, 0, (1U<<mi->b) * sizeof(mm_idx_bucket_t));
	memset(mi->S, 0, (mi->sum_len + 7) / 8 * 4);
	// memset(mi->seq, 0, n * sizeof(mm_idx_seq_t));
}

void mm_idx_post_nextdenovo(mm_idx_t *mi, int n_threads){
	mm_idx_post(mi, n_threads);
}

void mm_idx_str_nextdenovo2(mm_idx_t *mi, const char *name, const uint8_t *s, const uint32_t s_l, const int i)
{
	if (!i) mi->seq[i].offset = 0;
	while (mi->sum_len < mi->seq[i].offset + s_l){
		uint64_t old_max_len = (mi->sum_len + 7) / 8;
		mi->sum_len += 1000000;
		uint64_t max_len = (mi->sum_len + 7) / 8;
		mi->S = (uint32_t*)realloc(mi->S, max_len * 4);
		memset(&mi->S[old_max_len], 0, 4 * (max_len - old_max_len));
	}
	...
	mi->n_seq ++;
	mm_idx_seq_t *p = &mi->seq[i];
	// p->name = (char*)kmalloc(mi->km, strlen(name) + 1);
	// strcpy(p->name, name);
	// kh_put(str, (khash_t(str) *)mi->h, p->name, &absent);
	// assert(absent);

	p->len = s_l;
	for (j = 0; j < p->len; ++j) {
		o = p->offset + j;
		mm_seq4_set(mi->S, o, s[j]);
	}
	a.n = 0;
	mm_sketch_nextdenovo(0, s, p->len, mi->w, mi->k, i, mi->flag & MM_I_HPC, &a);
	mm_idx_add(mi, a.n, a.a);
	free(a.a);
	mi->seq[i + 1].offset = p->offset + s_l;
}

void mm_idx_str_nextdenovo3(mm_idx_t *mi, const char *s, const uint32_t s_l)
{
	while (mi->sum_len < s_l){
		uint64_t old_max_len = (mi->sum_len + 7) / 8;
		mi->sum_len += 1000000;
		uint64_t max_len = (mi->sum_len + 7) / 8;
		mi->S = (uint32_t*)realloc(mi->S, max_len * 4);
	...
	mm128_v a = {0,0,0};
	mi->n_seq = 1;
	mm_idx_seq_t *p = &mi->seq[0];
	
	p->offset = 0;
	p->len = s_l;
	for (j = 0; j < p->len; ++j) {
		mm_seq4_set(mi->S, j, seq_nt4_table[(uint8_t)s[j]]);
	}
	a.n = 0;
	mm_sketch(0, s, p->len, mi->w, mi->k, 0, mi->flag & MM_I_HPC, &a);
	mm_idx_add(mi, a.n, a.a);
	free(a.a);
	mm_idx_post(mi, 1);
}

mm_idx_t *mm_idx_str_nextdenovo1(int w, int k, int is_hpc, int bucket_bits, const char *seq, uint32_t sum_len)
{
	mm128_v a = {0,0,0};
	mm_idx_t *mi;
	int flag = 0;
	...
	mi = mm_idx_init(w, k, bucket_bits, flag);
	mi->n_seq = 1;
	mi->seq = (mm_idx_seq_t*)kcalloc(mi->km, 1, sizeof(mm_idx_seq_t)); // ->seq is allocated from km
	mi->S = (uint32_t*)calloc((sum_len + 7) / 8, 4);
	mm_idx_seq_t *p = &mi->seq[0];
	uint32_t j;
	p->offset = 0;
	p->len = sum_len;
	for (j = 0; j < p->len; ++j) {
		mm_seq4_set(mi->S, j, seq_nt4_table[(uint8_t)seq[j]]);
	}
	a.n = 0;
	mm_sketch(0, seq, p->len, w, k, 0, is_hpc, &a);
	mm_idx_add(mi, a.n, a.a);
	free(a.a);
	mm_idx_post(mi, 1);
	return mi;
