...
//This part intends to extend overlap region
extern idx_t *idx_ptr;
...
char *bit2seq(FILE *fp, int rev, uint64_t offset, uint32_t start, uint32_t end)
{
  ...
  for(i = 0; i < cnt; i++) {
    memcpy(buffer + (i << 4), bases_arr[tmp[i] >> 16 & 0xFFFF], 8);
    memcpy(buffer + (i << 4) + 8, bases_arr[tmp[i] & 0xFFFF], 8);
  }
  memcpy(ret, buffer + start - start_align, end - start + 1);
  if (rev) {
    char c1, c2;
    int l = end - start + 1;
    for(i = 0; i < l >> 1; ++i) {
      c1 = "ACGT"[3 - nt_table[(uint8_t)ret[i]]];
      c2 = "ACGT"[3 - nt_table[(uint8_t)ret[l - 1 - i]]];
      ret[i] = c2; ret[l - 1 - i] = c1;
    }
...
char *subseq(char *seq, int rev, int start, int end)
{
  char *ret = (char *)malloc(sizeof(char) * (end - start + 2));
  memcpy(ret, seq + start, end - start + 1);
  if (rev) {
    char c1, c2;
    int i, l;
    l = end - start + 1;
...
KHASH_MAP_INIT_INT(kmer, pos_t)

int *locate_ovl(char *qseq, char *tseq)
{
  ...
  for(i = 0; i < RANGE; i += 4) {
    kmer = 0;
    for(j = 0; j < 8; j++) kmer = kmer << 2 | nt_table[(uint8_t)tseq[i + j]];
    kmer = kmer & 0xFFFF;
    k = kh_put(kmer, h, kmer, &absent);
    if (absent) {
      kh_val(h, k).pos = (int *)malloc(sizeof(int) * 128);
      kh_val(h, k).cnt = 0;
    }
    kh_val(h, k).pos[kh_val(h, k).cnt] = i;
    kh_val(h, k).cnt += 1;
    kh_key(h, k) = kmer;
  }

  int match_cnt = 0;
  int diff_min, diff_range;
  int *ret = (int *)calloc(2, sizeof(int));
  int tseq_pos_last, qseq_pos_last;
  tseq_pos_last = qseq_pos_last = 0;

  for(i = 0; i < RANGE; i += 4) {
    kmer = 0;
    for(j = 0; j < 8; j++) kmer = kmer << 2 | nt_table[(uint8_t)qseq[i + j]];
    kmer = kmer && 0xFFFF;
    k = kh_get(kmer, h, kmer);
    if (k != kh_end(h)) {
      diff_min = RANGE;
      for(j = 0; j < kh_val(h, k).cnt; j++) {
        diff_range = abs(i - kh_val(h, k).pos[j]);
        if (diff_range <= DIFF) {
          if (diff_range <= diff_min) {
            diff_min = diff_range;
            tseq_pos_last = kh_val(h, k).pos[j];
            qseq_pos_last = i;
          }
        }
      }
      if (diff_min <= DIFF) ++match_cnt;
    }
  }
  if (match_cnt >= 20) {
    ret[0] = qseq_pos_last + 8;
    ret[1] = tseq_pos_last + 8;
  }
  for(k = kh_begin(h); k != kh_end(h); k++)
    if (kh_exist(h, k)) free(kh_val(h, k).pos);
  kh_destroy(kmer, h);
  return ret;
}

void extend_ovl(const mm_idx_t *mi, mm_reg1_t *r, mm_bseq1_t qseq_stat)
{
  khint_t k;
  FILE *fp;
  char *qseq, *tseq;
  ...
  k = kh_get(idx_offset, idx_ptr->h, atoi(mi->seq[r->rid].name));
  fp = fopen(idx_ptr->name, "rb");
  if (!r->rev) {
    if (r->qe + RANGE < qseq_stat.l_seq && r->re + RANGE < mi->seq[r->rid].len) {
      qseq = subseq(qseq_stat.seq, 0, r->qe, r->qe + RANGE);
      tseq = bit2seq(fp, 0, kh_val(idx_ptr->h, k), r->qe, r->qe + RANGE);
      pos = locate_ovl(qseq, tseq);
      r->qe += pos[0], r->re += pos[1];
      free(qseq); free(tseq); free(pos);
    }
    if (r->qs > RANGE && r->rs > RANGE) {
      qseq = subseq(qseq_stat.seq, 0, r->qs - RANGE, r->qs);
      tseq = bit2seq(fp, 0, kh_val(idx_ptr->h, k), r->rs - RANGE, r->rs);
      reverse(qseq, RANGE + 1); reverse(tseq, RANGE + 1);
      pos = locate_ovl(qseq, tseq);
      r->qs -= pos[0], r->rs -= pos[1];
      free(qseq); free(tseq); free(pos);
    }
  } else {
    if (r->qs > RANGE && r->re + RANGE < mi->seq[r->rid].len) {
      qseq = subseq(qseq_stat.seq, 1, r->qs - RANGE, r->qs);
      tseq = bit2seq(fp, 0, kh_val(idx_ptr->h, k), r->re, r->re + RANGE);
      pos = locate_ovl(qseq, tseq);
      r->qs -= pos[0], r->re += pos[1];
      free(qseq); free(tseq); free(pos);
    }
    if (r->qe + RANGE < qseq_stat.l_seq && r->rs > RANGE) {
      qseq = subseq(qseq_stat.seq, 1, r->qe, r->qe + RANGE);
      tseq = bit2seq(fp, 0, kh_val(idx_ptr->h, k), r->rs - RANGE, r->rs);
      reverse(qseq, RANGE + 1); reverse(tseq, RANGE + 1);
      pos = locate_ovl(qseq, tseq);
      r->qe += pos[0], r->rs -= pos[1];
      free(qseq); free(tseq); free(pos);
    }
  }
  fclose(fp);
...
static void worker_for(void *_data, long i, int tid) // kt_for() callback
{
  ...
  if (s->p->opt->mode) {
    int m, n;
    for (m = 0; m < s->n_seg[i]; ++m) {
      for (n = 0; n < s->n_reg[off + m]; ++n) {
        mm_reg1_t *r = &s->reg[off + m][n];
        extend_ovl(s->p->mi, r, s->seq[off + m]);
      }
    }
  }
}
