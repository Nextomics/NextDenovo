...
KHASH_MAP_INIT_INT(idx_offset, uint32_t)
...
void mm_sketch_nextdenovo(void *km, const uint8_t *str, int len, int w, int k, uint32_t rid, int is_hpc, mm128_v *p);
...
void mm_write_ovl(const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, const int);
